...
async function listMarkdownFilesIfExists(rootDir) {
...
async function getRoleContextFiles(role) {
    ...
    const governanceFiles = await Promise.all(roleGovernance[role].map(async (relativePath) => {
        const absolutePath = path.join(workspaceRoot, relativePath);
        return (await pathExists(absolutePath)) ? absolutePath : null;
    }));
    return dedupeAndSort([...roleFiles, ...governanceFiles.filter((value) => value !== null)]);
}
...
server.tool("get_business_analyst_context_index", "Lists all Markdown source documents in analysis/ used as business analyst context.", async () => {
    const files = (await listMarkdownFiles(analysisRoot)).sort((a, b) => a.localeCompare(b));
    const documentIndex = await Promise.all(files.map(async (filePath) => {
...
server.tool("get_architect_context_index", "Lists all Markdown source documents in architecture/ used as architect context.", async () => {
...
server.tool("get_business_analyst_context_bundle", "Loads latest business analyst documents from analysis/. Reads files at runtime so updates are automatically reflected.", {
    ...
    const selectedRelativePaths = paths && paths.length > 0
        ? paths.map((p) => p.replace(/\\/g, "/")).filter((p) => fileSet.has(p))
        : Array.from(fileSet).sort((a, b) => a.localeCompare(b));
    const documents = await Promise.all(selectedRelativePaths.map(async (relativePath) => {
        const absolutePath = path.join(workspaceRoot, relativePath);
...
server.tool("get_architect_context_bundle", "Loads latest architect documents from architecture/. Reads files at runtime so updates are automatically reflected.", {
    includeContent: z.boolean().default(true),
    maxCharsPerFile: z.number().int().positive().max(200000).default(20000),
    paths: z.array(z.string().min(1)).optional()
}, async ({ includeContent, maxCharsPerFile, paths }) => {
...
server.tool("get_role_context_index", "Lists all Markdown source documents in role-scoped context for the selected role.", {
    role: roleSchema
}, async ({ role }) => {
...
server.tool("get_role_context_bundle", "Loads latest role-scoped documents for the selected role. Reads files at runtime so updates are automatically reflected.", {
    role: roleSchema,
    includeContent: z.boolean().default(true),
    maxCharsPerFile: z.number().int().positive().max(200000).default(20000),
    paths: z.array(z.string().min(1)).optional()
...
server.tool("add_numbers", "Adds two numbers and returns the sum.", {
    a: z.number(),
    b: z.number()
}, async ({ a, b }) => {
    return {
...
server.tool("create_vat_claim_stub", "Creates a draft claim payload from VAT totals.", {
...
}, async ({ taxpayerId, periodStart, periodEnd, outputVatAmount, inputVatDeductibleAmount, adjustments }) => {
...
server.tool("validate_dk_vat_filing", "Validates Danish VAT filing fields and returns derived claim inputs.", {
    ...
    adjustments: z.number().default(0)
}, async ({ cvrNumber, taxPeriodStart, taxPeriodEnd, filingType, hadTaxableActivity, outputVatAmount, inputVatDeductibleAmount, vatOnGoodsPurchasesAbroadAmount, vatOnServicesPurchasesAbroadAmount, rubrikAGoodsEuPurchaseValue, rubrikAServicesEuPurchaseValue, rubrikBGoodsEuSaleValue, rubrikBServicesEuSaleValue, rubrikCOtherVatExemptSuppliesValue, adjustments }) => {
    const errors = [];
    const warnings = [];
    ...
    const monetaryTotal = outputVatAmount +
        inputVatDeductibleAmount +
        vatOnGoodsPurchasesAbroadAmount +
        vatOnServicesPurchasesAbroadAmount +
        Math.abs(adjustments);
    const internationalValueTotal = rubrikAGoodsEuPurchaseValue +
    ...
    if ((vatOnGoodsPurchasesAbroadAmount > 0 || vatOnServicesPurchasesAbroadAmount > 0) &&
    ...
    if ((rubrikBGoodsEuSaleValue > 0 || rubrikBServicesEuSaleValue > 0) &&
    ...
    const netVatAmount = outputVatAmount - inputVatDeductibleAmount + adjustments;
    const resultType = netVatAmount > 0 ? "payable" : netVatAmount < 0 ? "refund" : "zero";
    const periodDays = start && end ? Math.floor((end.getTime() - start.getTime()) / (24 * 60 * 60 * 1000)) + 1 : null;
    const result = {
...
server.tool("evaluate_dk_vat_filing_obligation", "Evaluates Danish VAT filing obligation, cadence, and return type from basic taxpayer facts.", {
...
}, async ({ isVatRegistered, annualTaxableTurnoverDkk, isNewBusiness, optedMonthly, optedQuarterly, hasActivityThisPeriod, hasSubmittedReturn, returnDueDatePassed, reportsEuSalesWithoutVat }) => {
    ...
    if (optedMonthly || annualTaxableTurnoverDkk > 50000000) {
        ...
        cadenceReason.push(optedMonthly ? "Taxpayer opted in for monthly reporting." : "Turnover above DKK 50M.");
    }
